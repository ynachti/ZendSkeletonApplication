...
function addspecfloor(id, label)
{
	if(locationcount>3) alert('You Have Select 3 Choices')
		...
		divtxt = "<li class=\"ui-state-default\" id=\"item_" + id + "\"><span class=\"ui-icon ui-icon-arrowthick-2-n-s\"></span>" + floorsdictionary[id] + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href=\"#\" onclick=\"removespecfloor(" + id + ");\"><i class=\"icon icon-trash\"></i></a> </li>";		
		$('#sortable').append(divtxt);		
...
function addlocroom(id)
{
	if(locationcount>3) alert('You Have Select 3 Choices')
		...
		divtxt = "<li class=\"ui-state-default\" id=\"item_" + id + "\"><span class=\"ui-icon ui-icon-arrowthick-2-n-s\"></span>" + buildingsdictionary[id] + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href=\"#\" onclick=\"removebuilding(\'" + id +  "\');\"><i class=\"icon icon-trash\"></i></a> </li>";		
	    $('#sortable').append(divtxt);																													                           
...
{	
	...
	$('location_set_ucc').removeClass('active');
...
function dspLocationGroup(id)
{
	...
	if(id=='1') {
		...
		$('#location_set_ucc').addClass('active');
...
$(document).ready(function(){
    ...
    $("#locationlist").click(function(){
        ...
        if($(".trigger").html()=='<i class="icon icon-eye-open icon-white"></i>&nbsp; <b>Show Preferences</b>'){
        	$(".trigger").html('<i class="icon icon-eye-close icon-white"></i>&nbsp; <b>Hide Preferences</b>');
        }
        else $(".trigger").html('<i class="icon icon-eye-open icon-white"></i>&nbsp; <b>Show Preferences</b>');
        
        return false;
    });

    
    $(".triggerold").click(function(){
        $(".panel").toggle("medium");
        $(this).toggleClass("active");
        if($(".trigger").html()=='<i class=\"icon icon-list icon-white\"></i>&nbsp;&nbsp;<i class=\"icon icon-chevron-right icon-white\"></i>'){
        	$(".trigger").html('<i class=\"icon icon-list icon-white\"></i>&nbsp;&nbsp;<i class=\"icon icon-chevron-down icon-white\"></i>');
        }
        else $(".trigger").html('<i class=\"icon icon-list icon-white\"></i>&nbsp;&nbsp;<i class=\"icon icon-chevron-right icon-white\"></i>');
        return false;
    });


    $("#buildingselector_1").change(function () {
    	prefs[0]=buildingsdictionary[$("#buildingselector_1 :selected").val()];
    	updateReviewPage();
    });
    $("#buildingselector_2").change(function () {
    	prefs[1]=buildingsdictionary[$("#buildingselector_2 :selected").val()];
    	updateReviewPage();
    });
    $("#buildingselector_3").change(function () {
    	prefs[2]=buildingsdictionary[$("#buildingselector_3 :selected").val()];
    	updateReviewPage();
    });

    $("#floorselector_1").change(function () {
    	prefs[0]=floorsdictionary[$("#floorselector_1 :selected").val()];
    	updateReviewPage();
    });
    $("#floorselector_2").change(function () {
    	prefs[1]=floorsdictionary[$("#floorselector_2 :selected").val()];
    	updateReviewPage();
    });
    $("#floorselector_3").change(function () {
    	prefs[2]=floorsdictionary[$("#floorselector_3 :selected").val()];
    	updateReviewPage();
    });
	...
	$("#sortable").sortable({
	    update: function(event, ui) {
		    ...
		    pageArr = $('#sortable').sortable('serialize').split("&");
	    	for ( var i = 0, l = pageArr.length; i < l; i++ ) {		    	
	    		prefids[i] = pageArr[i].substr(7,pageArr[i].length);
	    		if(preferencetype=='Building'){
	    		   prefs[i]=buildingsdictionary[prefids[i]];
	    		}
	    		else{
	    		   prefs[i]=floorsdictionary[prefids[i]];
	    		}
	    	}
	...
	function resetbuildingbuttons()
	{
    	...
    	$('#location_set_5').removeClass('active');
    	$('#location_set_ucc').removeClass('active');
    	$('#location_set_ucc').removeClass('btn-info');
    	$('#location_set_ucc').addClass('btn-default');
		...
		if(preflistfirstview==true){
			$(".panel").show();
			$(".panel").addClass('active');
			preflistfirstview=false;
		}
    ...
    $('#summer_location_set_1').click(function(){    	
    	resetbuildingbuttons();
    	$('#summer_location_set_1').removeClass('btn-default');
    	$('#summer_location_set_1').addClass('btn-info');		
    	$('#summer_location_set_1').addClass('active');
		$('#daytonfloorplans').fadeIn(250);		
    });
    ...
    $('#location_set_ucc').click(function(){
    	resetbuildingbuttons();
    	$('#location_set_ucc').removeClass('btn-default');
    	$('#location_set_ucc').addClass('btn-info');
    	$('#location_set_ucc').addClass('active');
